...
```{r}
ctrl <- trainControl(method = 'boot', 15)
lasso <- train(Temp ~ Wind + Month, data = train_regression, method = 'lasso', trControl = ctrl)
...
lasso_pred <- predict(lasso, newdata = test_regression)

plot_lasso_preds <- data.frame(temp_pred = lasso_pred, observed_temp = test_regression$Temp)

ggplot(data = plot_lasso_preds) +
  geom_point(aes(x=observed_temp, y = temp_pred)) +
  ggtitle("True Temp vs. Predicted Temp Lasso Regression") +
  geom_abline(intercept = 0, slope = 1, color = 'blue') +
  theme_bw()

#median residual value should be close to zero

lasso_resid <- median(resid(lasso))
print(paste0("Median residual value, ", lasso_resid, ", is close to zero"))
...
ggplot(pred_label, aes(x = 1:26, y = prediction, color = label))+
  geom_point()
...
# read in data and select for necessary columns

library(mlbench)
data(BreastCancer)

cancer <- BreastCancer %>% select(-Id)
head(cancer)
str(cancer)
...
```{r}
# remove up na values -- only 16 rows with na values so just deleted them from the data
print(dim(cancer))
...
feat1 <- ggplot(data = BreastCancer, aes(x = Cl.thickness, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat2 <- ggplot(data = BreastCancer, aes(x = Cell.size, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat3 <- ggplot(data = BreastCancer, aes(x = Cell.shape, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat4 <- ggplot(data = BreastCancer, aes(x = Marg.adhesion, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat5 <- ggplot(data = BreastCancer, aes(x = Epith.c.size, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat6 <- ggplot(data = BreastCancer, aes(x = Bare.nuclei, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat7 <- ggplot(data = BreastCancer, aes(x = Bl.cromatin, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat8 <- ggplot(data = BreastCancer, aes(x = Normal.nucleoli, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()
feat9 <- ggplot(data = BreastCancer, aes(x = Mitoses, fill = Class)) + 
  geom_histogram(position="identity", alpha=0.5, stat = "count")  +
  theme_bw()


grid.arrange(feat1, feat2, feat3, feat4, feat5, feat6, feat7, feat8, feat9)
```


```{r}
# convert ordinal values to numeric to generate correlation matrix
cancer_num <- cancer 
cancer_num <- cancer_num %>% transmute(as.numeric(Cl.thickness), as.numeric(Cell.size), as.numeric(Cell.shape),
                                       as.numeric(Marg.adhesion), as.numeric(Epith.c.size), as.numeric(Bare.nuclei),
                                       as.numeric(Bl.cromatin), as.numeric(Normal.nucleoli), as.numeric(Mitoses), Class)
str(cancer_num)
```


```{r}
# look at correlation matrix of data:
res <- cor(select(cancer_num, -Class))
round(res, 2)
```
...
train_size <- floor(0.75 * nrow(cancer))
set.seed(543)
train_pos <- sample(seq_len(nrow(cancer)), size = train_size)
train_set <- cancer[train_pos,]
test_set <- cancer[-train_pos,]

tumor_features <- c('Cl.thickness', 'Cell.size', 'Cell.shape', 'Marg.adhesion', 'Epith.c.size', 
...
**Selected Logistic regression because it is an effective binary classifier. LDA was not a good option here because all the features aren't normally distributed. Used regularized logistic regression to tune coefficients. Tuning coefficient, lamda, selected by the function**
```{r}
# set up crossvalidation with 15 folds:
ctrl <- trainControl(method = "repeatedcv", repeats = 15,classProbs = T, savePredictions = T)

# initialize and train model
logistic_regression <- train(Class~., data = train_set, 
                             method = "regLogistic", family= "binomial", trControl = ctrl, na.action = na.pass)
```
...
head(logistic_regression$pred)
```
```{r}
# Whats the area under the curve in the training set?
roc_data <- roc(predictor = logistic_regression$pred$benign, response = logistic_regression$pred$obs)
roc_data
```


**Testing on independent set:**
```{r}
# predict class (benign/malignant) in test set:
model_predict_class <- predict(logistic_regression, newdata = test_set)

# look at probabilities of classifications in test set:
model_predict_pred <- predict(logistic_regression, newdata = test_set, type = "prob")

# visualize ROC curve for model applied to test set
plot(x = roc(predictor = model_predict_pred$benign,
             response = test_set$Class)$specificities, 
     y = roc(predictor = model_predict_pred$benign, 
             response = test_set$Class)$sensitivities,
     col= "blue", xlim = c(1, 0), type ="l", ylab = "Sensitivity",
     xlab = "Specificity",
     main = "ROC curve for regularized logistic regression predicting malignancy")
legend("bottomright", legend = paste("Benign v Malignant --", 
                                     roc(predictor = model_predict_pred$benign,
                                         response = test_set$Class)$auc
, sep = ""), col = c("blue"), fill = c("blue"))

#confusion matrix
confusionMatrix(model_predict_class, 
